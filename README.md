...
**React + Vite**
コンポーネント指向でUIを構築でき、TypeScriptとの組み合わせによって保守性の高いフロントエンドを開発できるため採用しました。また、Viteによる高速な開発環境を利用し、効率的に開発できる構成としました。

**TailwindCSS + shadcn/ui**
CSSの設計・スタイリングにかかる時間を削減し、機能開発に集中するためTailwind CSSを採用しました。shadcn/uiでは、ログインフォームやカードなどのUIコンポーネントを活用し、デザインの統一と開発スピードの向上を図りました。

### バックエンド

**Ruby on Rails(APIモード)**
CRUD処理や認証などのAPIを効率的に構築できるため採用しました。また、Active Recordによるデータベース操作を通して、MVCフレームワークを用いたWeb API開発を経験することを目的としました。

**PostgreSQL**
本番運用を想定し、SQLiteではなくAWS RDSでの本格的なDBの構築・運用を経験するために選定しました。

### インフラ

**AWS EC2 + RDS + S3**
PaaS(Railway/Renderなど)を使えば簡単にデプロイできる中、インフラの基礎を実践的に学ぶため、あえてAWSを採用しました。VPCやセキュリティグループ、IAMの設定に加え、EC2上でのアプリケーションのデプロイやNginx、HTTPS化まで、自身で構築・運用することを目的としました。

**Route 53(独自ドメイン)**
`sailnote-app.com`を取得し、フロント(Vercel)とバックエンド(`api.`サブドメイン)の両方にDNSを設定。独自ドメイン+HTTPSでの公開までを一通り経験することを目指しました。
